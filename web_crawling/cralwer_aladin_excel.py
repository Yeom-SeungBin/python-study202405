...
# 브라우저 내부 대기
# t.sleep(10) -> 브라우저 로딩에 상관없이 무조건 10초 대기
...
# 셀 높이 조정
# worksheet.set_row(행 번호, 높이, 서식양식(format))
...
for col, header in enumerate(headers):
    # write(행 번호, 열 번호, 셀에 들어갈 문자열, 포맷 형식)
...
while cur_page_num <= target_page_num:
    ...
    for div_ss_book_box in div_list:
        ...
        try:
            # 이미지 바이트 변환 처리
            # BytesIO 객체 생성. 생성자의 매개값으로 아까 준비 해놓은 img 태그의 src 값을 urlopen 메서드에게 전달
        ...
        # 다음 행에 다음 데이터를 배치하기 위한 cnt 값 증가
        cnt += 1
